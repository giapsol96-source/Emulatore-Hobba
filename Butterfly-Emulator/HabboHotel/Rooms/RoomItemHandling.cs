...
namespace Butterfly.HabboHotel.Rooms
{
    class RoomItemHandling
    {
        ...
        internal void LoadFurniture()
        {
            ...
            {
                dbClient.setQuery(@"
            SELECT ir.item_id, ir.x, ir.y, ir.rot AS n, ir.base_id, IFNULL(ed.data, '') AS extradata
            FROM items_rooms AS ir
            LEFT JOIN items_extradata AS ed ON ed.item_id = ir.item_id
        ...
        ");
                ...
                {
                    itemID = Convert.ToUInt32(dRow["item_id"]);
                    x = Convert.ToDecimal(dRow["x"]);
                    y = Convert.ToDecimal(dRow["y"]);
                    n = Convert.ToSByte(dRow["n"]);
                    baseID = Convert.ToUInt32(dRow["base_id"]);
                    extradata = dRow["extradata"].ToString();
                ...
                // Gestione roller, moodlight e wired
        ...
        internal void SaveFurnitureToMSSQL(IQueryAdapter dbClient)
        {
            try
            {
                if (mAddedItems.Count > 0 || mRemovedItems.Count > 0 || mMovedItems.Count > 0 || room.GetRoomUserManager().PetCount > 0)
                {
                    ...
                    // Rimuovi furni eliminati
                    foreach (RoomItem Item in mRemovedItems.Values)
                    {
                        standardQueries.AddQuery("DELETE FROM items_rooms WHERE item_id = " + Item.Id + " AND room_id = " + room.RoomId);
                    }

                    // Inserisci nuovi furni
                    foreach (RoomItem Item in mAddedItems.Values)
                    {
                        // Salvataggio extra data
                        ...
                        if (Item.IsFloorItem)
                        {
                            ...
                            itemInserts.AddQuery(
                                "INSERT INTO items_rooms (item_id,room_id,x,y,n,base_id) VALUES (" + // <-- modificato base_item → base_id
                                Item.Id + "," + Item.RoomId + "," +
                                TextHandling.GetString(combinedCoords) + "," +
                                TextHandling.GetString(Item.GetZ) + "," +
                                Item.Rot + "," + Item.BaseItem + ")"); // <-- Item.BaseItem
                        }
                        else
                        {
                            itemInserts.AddQuery(
                                "INSERT INTO items_rooms (item_id,room_id,x,y,n,base_id) VALUES (" + // <-- base_id
                                Item.Id + "," + Item.RoomId + "," +
                                TextHandling.GetString(Item.wallCoord.GetXValue()) + "," +
                                TextHandling.GetString(Item.wallCoord.GetYValue()) + "," +
                                Item.wallCoord.n() + "," + Item.BaseItem + ")");
                        }
                    }

                    // Aggiorna furni mossi
                    foreach (RoomItem Item in mMovedItems.Values)
                    {
                        ...
                        if (Item.IsWallItem)
                        {
                            standardQueries.AddQuery(
                                "UPDATE items_rooms SET x=" + TextHandling.GetString(Item.wallCoord.GetXValue()) +
                                ", y=" + TextHandling.GetString(Item.wallCoord.GetYValue()) +
                                ", n=" + Item.wallCoord.n() +
                                ", base_id=" + Item.BaseItem + // <-- modificato
                        ...
                        else
                        {
                            double combinedCoords = TextHandling.Combine(Item.GetX, Item.GetY);
                            standardQueries.AddQuery(
                                "UPDATE items_rooms SET x=" + TextHandling.GetString(combinedCoords) +
                                ", y=" + TextHandling.GetString(Item.GetZ) +
                                ", n=" + Item.Rot +
                                ", base_id=" + Item.BaseItem + // <-- modificato
                                " WHERE item_id = " + Item.Id);
                        }
                    ...
                    // Salvataggio aggiornamenti dei pets
        ...
        internal void SaveFurniture(IQueryAdapter dbClient)
        {
            ...
            try
            {
                ...
                {
                    QueryChunk standardQueries = new QueryChunk();
                    QueryChunk itemInserts = new QueryChunk("REPLACE INTO items_rooms (item_id,room_id,x,y,n,base_id) VALUES "); // <-- aggiunto base_id
                    ...
                    if (mAddedItems.Count > 0)
                    {
                        foreach (RoomItem Item in mAddedItems.Values)
                        {
                            ...
                            {
                                double combinedCoords = TextHandling.Combine(Item.GetX, Item.GetY);
                                itemInserts.AddQuery("(" + Item.Id + "," + Item.RoomId + "," + TextHandling.GetString(combinedCoords) + "," + TextHandling.GetString(Item.GetZ) + "," + Item.Rot + "," + Item.BaseItem + ")"); // <-- aggiunto Item.BaseItem
                            }
                            else
                            {
                                itemInserts.AddQuery("(" + Item.Id + "," + Item.RoomId + "," + TextHandling.GetString(Item.wallCoord.GetXValue()) + "," + TextHandling.GetString(Item.wallCoord.GetYValue()) + "," + Item.wallCoord.n() + "," + Item.BaseItem + ")"); // <-- aggiunto Item.BaseItem
                            }
                        }
                    }
                    ...
                    foreach (RoomItem Item in mMovedItems.Values)
                    {
                        ...
                        if (Item.IsWallItem)
                        {
                            standardQueries.AddQuery("UPDATE items_rooms SET x=" + TextHandling.GetString(Item.wallCoord.GetXValue()) + ", y=" + TextHandling.GetString(Item.wallCoord.GetYValue()) + ", n=" + Item.wallCoord.n() + ", base_id=" + Item.BaseItem + " WHERE item_id = " + Item.Id); // <-- aggiunto base_id
                        }
                        else
                        {
                            double combinedCoords = TextHandling.Combine(Item.GetX, Item.GetY);
                            standardQueries.AddQuery("UPDATE items_rooms SET x=" + TextHandling.GetString(combinedCoords) + ", y=" + TextHandling.GetString(Item.GetZ) + ", n=" + Item.Rot + ", base_id=" + Item.BaseItem + " WHERE item_id = " + Item.Id); // <-- aggiunto base_id
                        }
                    }
