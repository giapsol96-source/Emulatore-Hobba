...
namespace Butterfly.HabboHotel.Rooms
{
    public class RoomUser : IEquatable<RoomUser>
    {
        ...
        internal void Chat(GameClient Session, string Message, bool Shout)
        {
            ...
            if (Message.StartsWith(":") && Session != null)
            {
                ...
                if (ChatCommandRegister.IsChatCommand(parsedCommand[0].ToLower().Substring(1)))
                {
                    ...
                    if (handler.WasExecuted())
                    {
                        ...
                        if (Session.GetHabbo().Rank > 5)
                        {
                            ButterflyEnvironment.GetGame().GetModerationTool().LogStaffEntry(
                                Session.GetHabbo().Username, string.Empty, "Chat command",
                                string.Format("Issued chat command {0}", Message)
                            );
                        }
                        return;
                    }
            ...
            if (!IsBot && Session?.GetHabbo() != null)
            {
                try
                {
                    using (IQueryAdapter dbClient = ButterflyEnvironment.GetDatabaseManager().getQueryreactor())
                    {
                        ...
                        string fullDate = now.ToString("yyyy-MM-dd HH:mm:ss");

                        string sql = "INSERT INTO chatlogs (user_id, room_id, hour, minute, full_date, timestamp, message, user_name) " +
                                     "VALUES (" + Session.GetHabbo().Id + ", " + GetRoom().RoomId + ", " + now.Hour + ", " + now.Minute +
                                     ", '" + fullDate + "', " + unixTimestamp + ", '" + Message.Replace("'", "''") +
                                     "', '" + Session.GetHabbo().Username.Replace("'", "''") + "')";

                        dbClient.runFastQuery(sql);
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine("Errore inserimento chatlog: " + ex.Message);
                }
            }
        ...
        {
            Console.WriteLine("[SetPos] PRIMA - X=" + this.X + ", Y=" + this.Y + ", SetX=" + this.SetX + ", SetY=" + this.SetY);
            Console.WriteLine("[SetPos] Ricevuto - pX=" + pX + ", pY=" + pY + ", pZ=" + pZ);

            this.X = pX;
            this.Y = pY;
            this.Z = pZ;

            Console.WriteLine("[SetPos] DOPO - X=" + this.X + ", Y=" + this.Y + ", SetX=" + this.SetX + ", SetY=" + this.SetY);

            if (isFlying)
            {
                Console.WriteLine("[SetPos] User è in volo, aggiungendo offset");
                Z += 4 + 0.5 * Math.Sin(0.7 * flyk);
                Console.WriteLine("[SetPos] DOPO volo - Z=" + this.Z);
            }

            this.UpdateNeeded = true;
            Console.WriteLine("[SetPos] UpdateNeeded settato a true");
        }
            ...
            try
            {
                if ((Message != null) && !this.IsSpectator)
                {
                    if (this.IsBot)
                    {
                        ...
                        Message.AppendStringWithBreak(TextHandling.GetString(this.Z));
                    ...
                    else if ((!this.IsBot && (this.GetClient() != null)) && (this.GetClient().GetHabbo() != null))
                    {
                        ...
                        Console.WriteLine("[Serialize] Serializzando user: " + habbo.Username + " a X=" + this.X + ", Y=" + this.Y + ", Z=" + this.Z);
                        ...
                        Message.AppendStringWithBreak(habbo.Gender != null ? habbo.Gender.ToLower() : "m");
                        Message.AppendInt32(-1);
                        if (habbo.userGroup != null)
                        {
                            Message.AppendInt32(habbo.userGroup.groupID);
                        }
                        else
                        {
                            Message.AppendInt32(-1);
                        ...
                        Console.WriteLine("[Serialize] Inviati nel packet: X=" + this.X + ", Y=" + this.Y);
                    }
                }
            }
            catch (Exception e)
            {
                Console.WriteLine("[Serialize] EXCEPTION: " + e.ToString());
