...
namespace Butterfly.HabboHotel.Rooms
{
    class RoomUserManager
    {
        ...
        internal void AddUserToRoom(GameClient Session, bool Spectator)
        {
            ...
            {
                dbClient.runFastQuery("INSERT INTO user_roomvisits (user_id,room_id,entry_timestamp,exit_timestamp,hour,minute) VALUES ("
                    + Session.GetHabbo().Id + ","
                    + room.RoomId + ","
                    + ButterflyEnvironment.GetUnixTimestamp() + ",0,"
                    + DateTime.Now.Hour + ","
                    + DateTime.Now.Minute + ")");
            }
        }
        ...
        {
            if (this == null || this.room == null)
                return;

            try
            {
                KeyValuePair<int, RoomUser> pair = (KeyValuePair<int, RoomUser>)sender;
                ...
                if (((user != null) && (user.GetClient() != null)) && (user.GetClient().GetHabbo() != null))
                {
                    ...
                    if ((session != null) && (session.GetHabbo() != null))
                    {
                        ...
                        if ((this.userCount >= this.room.UsersMax) && (user.GetClient().GetHabbo().Rank < 4))
                        {
                            message = new ServerMessage(0xe0);
                            message.AppendInt32(1);
                            session.SendMessage(message);
                            message = new ServerMessage(0x12);
                            session.SendMessage(message);
                        }
                        else
                        {
                            ...
                            {
                                Group userGroup = session.GetHabbo().userGroup;
                                if (!this.room.groups.ContainsKey(userGroup.groupID))
                                {
                                    this.room.groups.Add(userGroup.groupID, userGroup);
                                    message = new ServerMessage(0x135);
                                    message.AppendInt32(1);
                                    message.AppendInt32(userGroup.groupID);
                                    message.AppendStringWithBreak(userGroup.groupBadge);
                            ...
                            if (!user.IsSpectator)
                            {
                                Console.WriteLine("[OnUserAdd] ===== INIZIO POSIZIONAMENTO =====");
                                Console.WriteLine("[OnUserAdd] Username: " + session.GetHabbo().Username);
                                Console.WriteLine("[OnUserAdd] Room ID: " + this.room.RoomId);
                                Console.WriteLine("[OnUserAdd] Room Type: " + this.room.Type);

                                DynamicRoomModel model = this.room.GetGameMap().Model;

                                Console.WriteLine("[OnUserAdd] SetDoorPos PRIMA: " + session.SetDoorPos);
                                Console.WriteLine("[OnUserAdd] room.IsPublic: " + this.room.IsPublic);
                                Console.WriteLine("[OnUserAdd] Model è null?: " + (model == null));

                                if (model != null)
                                {
                                    Console.WriteLine("[OnUserAdd] Model.DoorX: " + model.DoorX);
                                    Console.WriteLine("[OnUserAdd] Model.DoorY: " + model.DoorY);
                                    Console.WriteLine("[OnUserAdd] Model.DoorZ: " + model.DoorZ);
                                    Console.WriteLine("[OnUserAdd] Model.DoorOrientation: " + model.DoorOrientation);
                                }

                                if (this.room.IsPublic && session.SetDoorPos)
                                {
                                    Console.WriteLine("[OnUserAdd] >>> RAMO: Posizionamento con SetDoorPos (Room Link)");
                                    Console.WriteLine("[OnUserAdd] newDoorPos.X: " + session.newDoorPos.X);
                                    Console.WriteLine("[OnUserAdd] newDoorPos.Y: " + session.newDoorPos.Y);
                                    user.SetPos(session.newDoorPos.X, session.newDoorPos.Y, (double)this.room.GetGameMap().StaticModel.SqFloorHeight[session.newDoorPos.X, session.newDoorPos.Y]);
                                    user.SetRot(model.DoorOrientation, false);
                                    session.SetDoorPos = false;
                                }
                                else
                                {
                                    Console.WriteLine("[OnUserAdd] >>> RAMO: Posizionamento normale (porta stanza)");
                                    if (model != null)
                                    {
                                        Console.WriteLine("[OnUserAdd] Setando posizione: X=" + model.DoorX + ", Y=" + model.DoorY + ", Z=" + model.DoorZ);
                                        user.SetPos(model.DoorX, model.DoorY, model.DoorZ);
                                        user.SetRot(model.DoorOrientation, false);
                                    }
                                    else
                                    {
                                        Console.WriteLine("[OnUserAdd] ERRORE: Model è NULL!");
                                    }
                                }

                                Console.WriteLine("[OnUserAdd] SetDoorPos DOPO: " + session.SetDoorPos);
                                Console.WriteLine("[OnUserAdd] User posizionato a: X=" + user.X + ", Y=" + user.Y + ", SetX=" + user.SetX + ", SetY=" + user.SetY);
                                if (this.room.CheckRights(session, true))
                                {
                                    user.AddStatus("flatcrtl", "useradmin");
                                }
                                else if (this.room.CheckRights(session))
                                {
                                ...
                                {
                                    RoomItem item = this.room.GetRoomItemHandler().GetItem(user.GetClient().GetHabbo().TeleporterId);
                                    if (item != null)
                                    {
                                        Console.WriteLine("[OnUserAdd] Teleporting a item: X=" + item.GetX + ", Y=" + item.GetY + ", Z=" + item.GetZ);
                                ...
                                ServerMessage message2 = new ServerMessage(0x1c);
                                message2.AppendInt32(1);
                                user.Serialize(message2, this.room.GetGameMap().gotPublicPool);
                                this.room.SendMessage(message2);

                                Console.WriteLine("[OnUserAdd] Messaggio 28 (enter) inviato");
                                Console.WriteLine("[OnUserAdd] ===== FINE POSIZIONAMENTO =====");

                                if (!(!(this.room.Owner != session.GetHabbo().Username) || this.room.IsPublic))
                                {
                            ...
                            if (!user.IsSpectator)
                            {
                                foreach (RoomUser user2 in this.UserList.Values)
                                {
                                    if (user2 == null)
                                        continue;
                                    ...
                                    if (user.IsBot)
                                    {
                                        user2.BotAI.OnUserEnterRoom(user);
                                    }
                                }
                            }
                            ...
                            if (this.OnUserEnter != null)
                                this.OnUserEnter(user, null);

                            if (this.room.GotMusicController())
                                this.room.GetRoomMusicController().OnNewUserEnter(user);
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine("[OnUserAdd] EXCEPTION: " + exception.ToString());
                Logging.LogCriticalException(exception.ToString());
            }
        }
