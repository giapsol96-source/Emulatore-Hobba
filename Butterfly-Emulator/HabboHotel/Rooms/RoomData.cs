...
namespace Butterfly.HabboHotel.Rooms
{
    class RoomData
    {
        ...
        internal void Serialize(ServerMessage Message, bool ShowEvents)
        {
            ...
            // Controlla se c'è un evento e se vogliamo mostrarlo
            if (string.IsNullOrEmpty(EventName) || !ShowEvents)
            {
                // Nessun evento: serializza i dati standard della stanza
                Message.AppendBoolean(false);
                Message.AppendStringWithBreak(Name);
                Message.AppendStringWithBreak(Owner);
                Message.AppendInt32(State);       // Stato stanza
                Message.AppendInt32(UsersNow);    // Utenti attuali
                Message.AppendInt32(UsersMax);    // Max utenti
                Message.AppendStringWithBreak(Description);
                Message.AppendBoolean(true);      // ?
                Message.AppendBoolean(true);      // Può fare trade?
                Message.AppendInt32(Score);
                Message.AppendInt32(Category);
                Message.AppendStringWithBreak(""); // Campo vuoto
            ...
            {
                // Evento presente: serializza dati dell'evento
                Message.AppendBoolean(true);
                Message.AppendStringWithBreak(EventName);
                Message.AppendStringWithBreak(Owner);
                Message.AppendInt32(State);
                Message.AppendInt32(UsersNow);
                Message.AppendInt32(UsersMax);
                Message.AppendStringWithBreak(EventDescription);
                Message.AppendBoolean(true);  // ?
                Message.AppendBoolean(true);  // Può fare trade?
                Message.AppendInt32(Score);
                Message.AppendInt32(EventCategory);
                Message.AppendStringWithBreak(EventStartTime);
                Message.AppendInt32(TagCount);
            ...
            Message.AppendBoolean(true); // Fine serializzazione
        }
    }
}
