...
namespace Butterfly.HabboHotel.Users
{
    class Habbo
    {
        ...
        internal Habbo(UInt32 Id, string Username, string RealName,
          ...
          bool MutantPenalty, bool HasFriendRequestsDisabled, uint currentQuestID, int currentQuestProgress, DataRow groupRow, int achievementPoints, string auth_ticket)
        {
            this.Id = Id;
            ...
            if (groupRow != null)
            {
                ...
                // Prendere l'ID corretto del gruppo dal DB
                if (groupRow["id"] != DBNull.Value)   // <-- cambiato da "userid" a "id"
                    groupID = Convert.ToInt32(groupRow["id"]);

                if (groupRow["description"] != DBNull.Value)
                    description = (string)groupRow["description"];

                if (groupRow["badge"] != DBNull.Value)
                    badge = (string)groupRow["badge"];
        ...
        internal string GetQueryString
        {
            ...
            {
                HabboinfoSaved = true;
                return "UPDATE users SET users.last_online = '" + DateTime.Now.ToString() + "', activity_points = '" + ActivityPoints + "', activity_points_lastupdate = '" + LastActivityPointsUpdate + "', credits = '" + Credits + "', achievement_points = " + AchievementPoints + ", online = '0' WHERE id = '" + Id + "';";
            }
        }
        ...
        internal void OnDisconnect()
        {
            ...
            if (!HabboinfoSaved)
            {
                ...
                {
                    dbClient.runFastQuery("UPDATE users SET users.last_online = '" + DateTime.Now.ToString() + "', activity_points = " + ActivityPoints + ", activity_points_lastupdate = '" + LastActivityPointsUpdate + "', credits = " + Credits + ", achievement_points = " + AchievementPoints + ", online = '0' WHERE id = " + Id + " ;");
                }
            }
