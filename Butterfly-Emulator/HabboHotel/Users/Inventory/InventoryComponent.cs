...
namespace Butterfly.HabboHotel.Users.Inventory
{
    class InventoryComponent
    {
        ...
        internal void LoadInventory()
        {
            ...
            {
                id = Convert.ToUInt32(Row["id"]);         // ID della riga in user_items
                baseitem = Convert.ToUInt32(Row["base_item"]); // codice del furno
                extradata = DBNull.Value.Equals(Row["extra_data"]) ? string.Empty : (string)Row["extra_data"];
            ...
            using (IQueryAdapter dbClient = ButterflyEnvironment.GetDatabaseManager().getQueryreactor())
            {
                dbClient.setQuery("SELECT id, user_id, room_id, name, type, race, color, expirience, energy, nutrition, respect, createstamp, x, y, z " +
                                  "FROM user_pets WHERE user_id = @userid AND room_id = 0");
        ...
        internal UserItem AddNewItem(uint Id, uint BaseItem, string ExtraData, bool insert, bool fromRoom, uint songID = 0)
        {
            isUpdated = false;
            uint newItemId = Id;
            ...
            if (insert)
            {
                using (IQueryAdapter dbClient = ButterflyEnvironment.GetDatabaseManager().getQueryreactor())
                {
                    // Inserimento nel DB con AUTO_INCREMENT
                    dbClient.setQuery("INSERT INTO user_items (user_id, base_item, extra_data, room_id, x, y, z, rot, wall_pos) " +
                                      "VALUES (@userid, @baseid, @extradata, 0, 0, 0, 0, 0, '')");
                    dbClient.addParameter("userid", UserId);
                    dbClient.addParameter("baseid", BaseItem);
                    dbClient.addParameter("extradata", ExtraData);

                    newItemId = (uint)dbClient.insertQuery(); // id generato automaticamente
                }
            }

            UserItem itemToAdd = new UserItem(newItemId, BaseItem, ExtraData);

            // Aggiunta ai dizionari inventario
            if (itemToAdd.GetBaseItem().InteractionType == InteractionType.musicdisc)
                discs.Add(newItemId, itemToAdd);
            else if (itemToAdd.isWallItem)
                wallItems.Add(newItemId, itemToAdd);
            else
                floorItems.Add(newItemId, itemToAdd);

            if (!mAddedItems.ContainsKey(newItemId))
                mAddedItems.Add(newItemId, itemToAdd);

            if (mRemovedItems.Contains(newItemId))
                mRemovedItems.Remove(newItemId);
