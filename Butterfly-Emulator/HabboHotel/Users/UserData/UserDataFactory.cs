    ...
    class UserDataFactory
    {
        internal static UserData GetUserData(string sessionTicket, string ip, out byte errorCode)
        {
            ...
            using (IQueryAdapter dbClient = ButterflyEnvironment.GetDatabaseManager().getQueryreactor())
            {
                ...
                dbClient.setQuery("SELECT id, base_item, extra_data FROM user_items WHERE user_id = " + userID);
                ...
                {
                    dbClient.setQuery("SELECT group_memberships.*, groups.* " +
                  "FROM group_memberships " +
                  "LEFT JOIN groups ON group_memberships.groupid = groups.id " +
                  "WHERE group_memberships.userid = @userid " +
                  "AND group_memberships.is_current = '1'");
