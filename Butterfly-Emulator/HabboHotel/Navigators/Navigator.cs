...
namespace Butterfly.HabboHotel.Navigators
{
    class Navigator
    {
        ...
        // Restituisce solo le stanze pubbliche, escludendo le categorie
        internal IEnumerable<PublicItem> GetPublicRooms()
        {
            return PublicItems.Values.Where(p => !p.Category);
        }
        ...
        internal static ServerMessage SerializeEventListing(int Category)
        {
            ...
            {
                pair.Key.Serialize(Message, true); // ⚡ ShowEvents = true
            }
        ...
        private ServerMessage SerializeActiveRooms(int category)
        {
            ...
            // ⚡ Passiamo SOLO i RoomData, così la serializzazione è uniforme
            SerializeNavigatorRooms(ref reply, rooms.Select(x => x.Key).ToList());
        ...
        {
            reply.AppendStringWithBreak(string.Empty);  // header vuoto
            reply.AppendInt32(rooms.Count);             // numero stanze
            ...
            foreach (RoomData data in rooms)
            {
                ...
                // ⚡ FIX: Se il room_model ha PublicItems, serializza come stanza ufficiale
                RoomModel model = ButterflyEnvironment.GetGame().GetRoomManager().GetModel(data.ModelName, data.Id);
                bool hasPublicItems = (model != null && model.PublicItems != null && model.PublicItems.Count > 0);

                data.Serialize(reply, hasPublicItems);
            }
        }
        ...
        private void SerializeNavigatorRooms(ref ServerMessage reply, KeyValuePair<RoomData, int>[] rooms)
        {
            ...
            foreach (KeyValuePair<RoomData, int> pair in rooms)
            {
                ...
                // ⚡ FIX: Se il room_model ha PublicItems, serializza come stanza ufficiale
                RoomModel model = ButterflyEnvironment.GetGame().GetRoomManager().GetModel(pair.Key.ModelName, pair.Key.Id);
                bool hasPublicItems = (model != null && model.PublicItems != null && model.PublicItems.Count > 0);

                pair.Key.Serialize(reply, hasPublicItems);
            }
        }
