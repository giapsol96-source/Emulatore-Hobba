...
namespace Butterfly.HabboHotel.Events
{
    class EventManager
    {
        ...
        {
            KeyValuePair<RoomData, int>[] rooms;

            if (categoryID > 0 && eventCategories.ContainsKey(categoryID))
                rooms = eventCategories[categoryID].GetActiveRooms();
            else
                rooms = orderedEventRooms.ToArray();

            // ⚡ Copia Room.Event direttamente dalla stanza live
            for (int i = 0; i < rooms.Length; i++)
            {
                Room room = ButterflyEnvironment.GetGame().GetRoomManager().GetRoom(rooms[i].Key.Id);
                if (room != null && room.Event != null)
                {
                    rooms[i].Key.Event = room.Event;
                }
            }
        ...
        public void AddEventImmediate(RoomData data)
        {
            lock (this)
            {
                // Aggiunge la stanza agli eventi live se non è già presente
                if (!events.ContainsKey(data))
                    events.Add(data, data.UsersNow);

                // Aggiorna subito la lista ordinata per il navigatore
