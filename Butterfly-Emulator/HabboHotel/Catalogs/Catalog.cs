...
namespace Butterfly.HabboHotel.Catalogs
{
    class Catalog
    {
        ...
        internal void HandlePurchase(GameClient Session, int PageId, uint ItemId, string ExtraData, Boolean IsGift, string GiftUser, string GiftMessage, int Amount)
        {
            CatalogPage Page;
            ...
            {
                DeliverItems(Session, Item.GetBaseItem(), (Item.Amount * Session.GetHabbo().buyItemLoop), ExtraData, Item.songID);
            }
        }
        ...
        {
            Pet pet = new Pet(404, UserId, 0, Name, (uint)Type, Race, Color, 0, 100, 100, 0, ButterflyEnvironment.GetUnixTimestamp(), 0, 0, 0.0);
