    ...
    class GameClient
    {
        ...
        internal bool tryLogin(string AuthTicket)
        {
            try
            {
                ...
                {
                   // uint UserID = Convert.ToUInt32(userData.userID);
                   // ButterflyEnvironment.GetGame().GetClientManager().LogClonesOut(UserID);
                    ...
                    //SendNotifWithScroll("USER ID: " + UserID);
                    return false;
        ...
        internal void Stop()
        {
            if (GetHabbo() != null)
            {
                // Aggiorna il login_timestamp e setta online = 0
                using (IQueryAdapter dbClient = ButterflyEnvironment.GetDatabaseManager().getQueryreactor())
                {
                    uint userID = GetHabbo().Id;
                    ...
                    if (dbClient.dbType == Database_Manager.Database.DatabaseType.MySQL)
                        dbClient.runFastQuery("UPDATE users SET online = '0' WHERE id = '" + userID + "'");
                    else
                        dbClient.runFastQuery("UPDATE users SET online = '0' WHERE id = '" + userID + "'");
                }
