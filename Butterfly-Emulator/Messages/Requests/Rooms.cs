...
namespace Butterfly.Messages
{
    partial class GameClientMessageHandler
    {
        ...
        internal void GetRoomData3()
        {
            ...
            // ⚡ IMPORTANTE: Invia SEMPRE il messaggio (30) con i furni prestabiliti
            Response.Init(30);
            ...
            // Carica i furni dal database (solo per stanze private)
        ...
        internal void PrepareRoomForUser(uint Id, string Password)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        Room OldRoom = ButterflyEnvironment.GetGame().GetRoomManager().GetRoom(this.Session.GetHabbo().CurrentRoomId);
                        if (OldRoom != null)
                        {
                            if (OldRoom.GetRoomUserManager() != null)
                                OldRoom.GetRoomUserManager().RemoveUserFromRoom(this.Session, false, false);
                        }
                    }
                    ...
                    {
                        Console.WriteLine("[PrepareRoomForUser] Stanza pubblica ID: " + Id + ", ModelName: " + room2.ModelName);

                        if (!((room2.State <= 0) || this.Session.GetHabbo().HasFuse("fuse_mod")))
                        {
                            this.Session.SendNotif(LanguageLocale.GetValue("room.noaccess"));
                            this.Response.Init(0x12);
                            message.appendResponse(this.GetResponse());
                            message.sendResponse();
                            return;
                        ...
                        // ✅ SOLUZIONE 1: Forza il reload con timestamp
                        this.Response.Init(0xa5); // Packet 165 - unload model
                        this.Response.AppendStringWithBreak(room2.ModelName);
                        Console.WriteLine("[PrepareRoomForUser] Inviando packet 165 per scaricare modello dalla cache: " + room2.ModelName);
                        message.appendResponse(this.GetResponse());

                        // ✅ POI RICARICA CON TIMESTAMP (forza reload)
                        this.Response.Init(0xa6);
                        string modelPath = "/client/public/" + room2.ModelName + "/0?v=" + DateTime.Now.Ticks;
                        this.Response.AppendStringWithBreak(modelPath);
                        Console.WriteLine("[PrepareRoomForUser] Inviando packet 166 con path: " + modelPath);
                        message.appendResponse(this.GetResponse());
                    }
                    else if (room2.Type == "private")
                    {
                        Console.WriteLine("[PrepareRoomForUser] Stanza privata ID: " + Id);

                        this.Response.Init(0x13);
                        message.appendResponse(this.GetResponse());
                        if ((!this.Session.GetHabbo().HasFuse("fuse_enter_any_room") && !room2.CheckRights(this.Session, true)) && !this.Session.GetHabbo().IsTeleporting)
                        {
                            if (room2.State == 1)
                            {
                                ...
                                {
                                    this.Response.Init(0x83);
                                    message.appendResponse(this.GetResponse());
                                }
                                else
                                {
                                    this.Response.Init(0x5b);
                                    this.Response.AppendStringWithBreak("");
                                    message.appendResponse(this.GetResponse());
                                    ServerMessage message2 = new ServerMessage(0x5b);
                                    message2.AppendStringWithBreak(this.Session.GetHabbo().Username);
                                    room2.SendMessageToUsersWithRights(message2);
                                }
                                message.sendResponse();
                                return;
                            }
                        ...
                        this.Response.Init(0xa6);
                        this.Response.AppendStringWithBreak("/client/internal/" + room2.RoomId + "/id");
                        Console.WriteLine("[PrepareRoomForUser] Inviando packet 166 con path: /client/internal/" + room2.RoomId + "/id");
                        message.appendResponse(this.GetResponse());
                    }

                    this.Session.GetHabbo().LoadingChecksPassed = true;
                    Console.WriteLine("[PrepareRoomForUser] LoadingChecksPassed = true");

                    Console.WriteLine("[PrepareRoomForUser] Aggiungendo LoadRoomForUser response");
                    message.addBytes(this.LoadRoomForUser().getPacket);
                    message.sendResponse();

                    Console.WriteLine("[PrepareRoomForUser] Response inviato!");
                }
            }
            catch (Exception e)
            {
                Console.WriteLine("[PrepareRoomForUser] ERRORE: " + e.ToString());
        ...
        internal QueuedServerMessage LoadRoomForUser()
        {
            ...
            {
                Console.WriteLine("[LoadRoomForUser] Room is null o LoadingChecksPassed è false!");
                return response;
            }

            Console.WriteLine("[LoadRoomForUser] ===== INIZIO LOAD ROOM =====");
            Console.WriteLine("[LoadRoomForUser] Caricamento stanza " + Room.RoomId + ", Type: " + Room.Type + ", ModelName: " + Room.ModelName);

            // ✅ PULISCI LA CACHE - Invia packet 28 PRIMA
            Response.Init(28);
            Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 28 (clear users) ===");
            response.appendResponse(GetResponse());

            Response.Init(309);
            Response.AppendInt32(1);
            Response.AppendInt32(122768);
            Response.AppendStringWithBreak("b1201Xs03097s55091s17094a7a396e8d44670744d87bf913858b1fc");
            Console.WriteLine("[LoadRoomForUser] Inviando packet 309");
            response.appendResponse(GetResponse());

            Response.Init(69);
            Response.AppendStringWithBreak(Room.ModelName);
            Response.AppendUInt(Room.RoomId);
            Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 69 ===");
            Console.WriteLine("[LoadRoomForUser] ModelName: '" + Room.ModelName + "' (lunghezza: " + Room.ModelName.Length + ")");
            Console.WriteLine("[LoadRoomForUser] RoomId: " + Room.RoomId);
            Console.WriteLine("[LoadRoomForUser] === FINE PACKET 69 ===");
            response.appendResponse(GetResponse());

            if (Session.GetHabbo().SpectatorMode)
            {
                Response.Init(254);
                Console.WriteLine("[LoadRoomForUser] Inviando packet 254 (SpectatorMode)");
                response.appendResponse(GetResponse());
            }

            // ✅ PER LE STANZE PUBBLICHE
            if (Room.Type == "public")
            {
                Console.WriteLine("[LoadRoomForUser] Stanza pubblica");

                // ⚡ Invia gli utenti nella stanza PRIMA!
                var users = Room.GetRoomUserManager().GetRoomUsers();
                if (users != null && users.Count > 0)
                {
                    Response.Init(86); // Packet 86 per lista utenti
                    Response.AppendInt32(users.Count);
                    Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 86 (PRIMA) ===");
                    Console.WriteLine("[LoadRoomForUser] Numero utenti: " + users.Count);

                    foreach (var user in users)
                    {
                        if (user != null)
                        {
                            user.Serialize(Response, true);
                            Console.WriteLine("[LoadRoomForUser] - Utente: " + user.GetUsername());
                        }
                    }
                    response.appendResponse(GetResponse());
                    Console.WriteLine("[LoadRoomForUser] === FINE PACKET 86 ===");
                }
                else
                {
                    Console.WriteLine("[LoadRoomForUser] Nessun utente nella stanza pubblica");
                }

                // ✅ POI INVIA IL PACKET 30 (StaticFurniMap)
                Response.Init(30);
                if (Room.GetGameMap() != null && Room.GetGameMap().StaticModel != null && !string.IsNullOrEmpty(Room.GetGameMap().StaticModel.StaticFurniMap))
                {
                    Response.AppendStringWithBreak(Room.GetGameMap().StaticModel.StaticFurniMap);
                    Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 30 con StaticFurniMap ===");
                }
                else
                {
                    Response.AppendInt32(0);
                    Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 30 (vuoto) ===");
                }
                response.appendResponse(GetResponse());

                Console.WriteLine("[LoadRoomForUser] Stanza pubblica - ritorno");
                Console.WriteLine("[LoadRoomForUser] ===== FINE LOAD ROOM (PUBLIC) =====");
                return response;
            }

            // ✅ SOLO PER STANZE PRIVATE - invia item
            if (Room.Type == "private")
            {
                Console.WriteLine("[LoadRoomForUser] Stanza privata, caricamento dati privati...");

                // ⚡ Packet 70 - Floor Items
                Response.Init(70);
                var itemHandler = Room.GetRoomItemHandler();
                if (itemHandler != null)
                {
                    var floorItems = itemHandler.mFloorItems.Values.ToList();
                    Response.AppendInt32(floorItems.Count);
                    Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 70 ===");
                    Console.WriteLine("[LoadRoomForUser] Inviando " + floorItems.Count + " floor items");

                    foreach (var item in floorItems)
                    {
                        item.Serialize(Response);
                    }
                    Console.WriteLine("[LoadRoomForUser] === FINE PACKET 70 ===");
                }
                else
                {
                    Response.AppendInt32(0);
                    Console.WriteLine("[LoadRoomForUser] ItemHandler è null!");
                }
                response.appendResponse(GetResponse());

                // ⚡ Packet 71 - Wall Items
                Response.Init(71);
                if (itemHandler != null)
                {
                    var wallItems = itemHandler.mWallItems.Values.ToList();
                    Response.AppendInt32(wallItems.Count);
                    Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 71 ===");
                    Console.WriteLine("[LoadRoomForUser] Inviando " + wallItems.Count + " wall items");

                    foreach (var item in wallItems)
                    {
                        item.Serialize(Response);
                    }
                    Console.WriteLine("[LoadRoomForUser] === FINE PACKET 71 ===");
                }
                else
                {
                    Response.AppendInt32(0);
                ...
                if (Room.Wallpaper != "0.0")
                {
                    ...
                    Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 46 (wallpaper) ===");
                    response.appendResponse(GetResponse());
                }
                ...
                if (Room.Floor != "0.0")
                {
                    ...
                    Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 46 (floor) ===");
                    response.appendResponse(GetResponse());
                }
                ...
                Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 46 (landscape) ===");
                response.appendResponse(GetResponse());

                if (Room.CheckRights(Session, true))
                {
                    Response.Init(42);
                    Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 42 (owner) ===");
                    response.appendResponse(GetResponse());

                    Response.Init(47);
                    Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 47 (moderator) ===");
                    response.appendResponse(GetResponse());
                }
                ...
                if (Session.GetHabbo().RatedRooms.Contains(Room.RoomId) || Room.CheckRights(Session, true))
                {
                    ...
                    Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 345 (score: " + Room.Score + ") ===");
                }
                else
                {
                    Response.AppendInt32(-1);
                    Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 345 (score: -1) ===");
                }
                response.appendResponse(GetResponse());

                if (Room.HasOngoingEvent)
                {
                    Console.WriteLine("[LoadRoomForUser] Stanza ha evento in corso");
                ...
                else
                {
                    ...
                    Console.WriteLine("[LoadRoomForUser] === INVIANDO PACKET 370 (no event) ===");
                    response.appendResponse(GetResponse());
                }
            }

            Console.WriteLine("[LoadRoomForUser] Return response compilato");
            Console.WriteLine("[LoadRoomForUser] ===== FINE LOAD ROOM =====");
            return response;
        }
        ...
        internal void StartEvent()
        {
            ...
            // AGGIORNA IL ROOMDATA con i dati dell'evento
            Room.RoomData.EventName = Room.Event.Name;
            Room.RoomData.EventDescription = Room.Event.Description;
            Room.RoomData.EventCategory = Room.Event.Category;
            Room.RoomData.EventStartTime = Room.Event.StartTime;

            // Invia al client il messaggio di creazione evento (Info stanza o Navigator)
            Room.SendMessage(Room.Event.Serialize(Session));

            // Metti l'evento nella coda dell'EventManager
